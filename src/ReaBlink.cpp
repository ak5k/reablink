...
// BlinkEngine& BlinkEngine::GetInstance().= BlinkEngine::GetInstance();
...
bool GetEnabled()
{
    return BlinkEngine::GetInstance().GetLink().isEnabled();
}
...
void SetEnabled(bool enable)
{
    if (enable)
    {
        ...
        BlinkEngine::GetInstance().GetLink().enable(true);
        BlinkEngine::GetInstance().Initialize(true);
    }
    else
    {
        BlinkEngine::GetInstance().Initialize(false);
        BlinkEngine::GetInstance().GetLink().enable(false);
    }
}
...
{
    return BlinkEngine::GetInstance().GetStartStopSyncEnabled();
}
...
{
    BlinkEngine::GetInstance().SetStartStopSyncEnabled(enable);
}
...
int GetNumPeers()
{
    return (int)BlinkEngine::GetInstance().GetLink().numPeers();
}
...
double GetClockNow()
{
    return BlinkEngine::GetInstance().GetLink().clock().micros().count() /
           1.0e6;
}
...
double GetTempo()
{
    return BlinkEngine::GetInstance()
        .GetLink()
...
{
    BlinkEngine::GetInstance().SetTempo(bpm);
}
...
{
    auto sessionState =
        BlinkEngine::GetInstance().GetLink().captureAppSessionState();
    sessionState.setTempo(bpm, doubleToMicros(time));
    BlinkEngine::GetInstance().GetLink().commitAppSessionState(sessionState);
}
...
double GetBeatAtTime(double time, double quantum)
{
    return BlinkEngine::GetInstance()
        .GetLink()
        .captureAppSessionState()
        .beatAtTime(doubleToMicros(time), quantum);
}
...
double GetPhaseAtTime(double time, double quantum)
{
    return BlinkEngine::GetInstance()
        .GetLink()
        .captureAppSessionState()
        .phaseAtTime(doubleToMicros(time), quantum);
}
...
{
    return microsToDouble(BlinkEngine::GetInstance()
                              .GetLink()
                              .captureAppSessionState()
...
void startStop()
{
    ...
    {
        BlinkEngine::GetInstance().StopPlaying();
    }
    else
    {
        BlinkEngine::GetInstance().StartPlaying();
    }
}
...
{
    BlinkEngine::GetInstance().SetQuantum(quantum);
}
...
double GetQuantum()
{
    return BlinkEngine::GetInstance().GetQuantum();
}
...
{
    return BlinkEngine::GetInstance().SetMaster(enable);
}
...
bool GetMaster()
{
    return BlinkEngine::GetInstance().GetMaster();
}
...
{
    // BlinkEngine::GetInstance().Initialize(enable);
    return BlinkEngine::GetInstance().SetPuppet(enable);
}
...
bool GetPuppet()
{
    return BlinkEngine::GetInstance().GetPuppet();
}
...
bool runCommand(int command, int flag)
{
    ...
    if (GetEnabled())
    {
        ...
        if (command == 41130)
        {
            ...
            BlinkEngine::GetInstance().SetTempo(tempo - 1);
        }
        if (command == 41129)
        {
            res = true;
