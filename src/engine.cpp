...
AudioEngine::AudioEngine(Link &link)
    : mLink(link), mSharedEngineData({0., false, false, 4., false}), mLockfreeEngineData(mSharedEngineData),
      mIsPlaying(false)
{
}
...
double GetNextFullMeasureTimePosition()
{
    ...
    int measurePosition = (int)TimeMap2_timeToBeats(0, currentPosition, &measures3, &beats3, NULL,
...
int SetLaunchPrerollRegion()
{
    ...
    int measures3 = 0;
    int beats3 = 0;
    double num_measures = TimeMap2_timeToBeats(0, projectLength, &measures3, &beats3, NULL,
    ...
    num_measures = TimeMap2_timeToBeats(0, region_start, &measures3, &beats3, NULL,
                                        NULL); // Convert the project length to measures/beats
...
void AudioEngine::audioCallback(const std::chrono::microseconds hostTime, const std::size_t numSamples)
{
    ...
    if (isPuppet && !mIsPlaying && sessionState.isPlaying())
    {
        ...
        if (mLink.numPeers() > 0 && GetToggleCommandState(40620) == 0)
        {
            ...
            target_region_idx = AddProjectMarker2(0, true, pos_target, pos_target + (60. / tempo * engineData.quantum),
                                                  "reablink target", -1, 0);
            auto target_tempo_idx = FindTempoTimeSigMarker(0, pos_target);
            SetTempoTimeSigMarker(0, target_tempo_idx, pos_target, 0, 0, sessionState.tempo(), timesig_num,
                                  timesig_denom, false);
            ...
            CreateNewMIDIItemInProj(track, pos_preroll, pos_preroll + (60. / sessionState.tempo() * engineData.quantum),
                                    0);
